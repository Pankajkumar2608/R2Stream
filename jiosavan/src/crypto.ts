/**
 * JioSaavn URL Decryption
 * JioSaavn encrypts download URLs with DES (ECB mode, PKCS5 padding)
 */
...
function toBits(n: number, len: number): number[] {
  const bits = [];
  for (let i = len - 1; i >= 0; i--) bits[i] = n & 1, n >>= 1;
  return bits;
}
...
function feistel(r: number[], k: number[]): number[] {
  ...
  const xored = er.map((b, i) => b ^ k[i]);
  const sout: number[] = [];
  for (let i = 0; i < 8; i++) {
    const row = fromBits([xored[i*6], xored[i*6+5]]);
    const col = fromBits(xored.slice(i*6+1, i*6+5));
    sout.push(...toBits(S[i][row*16+col], 4));
  }
  return permute(sout, P);
}

function desDecryptBlock(block: number[], subkeys: number[][]): number[] {
  let bits = permute(block, IP);
  let l = bits.slice(0, 32), r = bits.slice(32);
  for (let i = 15; i >= 0; i--) {
    const tmp = r.slice();
    r = l.map((b, j) => b ^ feistel(r, subkeys[i])[j]);
    l = tmp;
  }
...
/**
 * Decrypt a JioSaavn encrypted media URL
 * Input: base64-encoded DES-encrypted URL from JioSaavn API
 * Output: plain HTTPS URL to the audio file
 */
export function decryptUrl(encryptedUrl: string): string {
  const keyBytes = Array.from(DES_KEY).map(c => c.charCodeAt(0));
  const keyBits: number[] = [];
  for (const byte of keyBytes) keyBits.push(...toBits(byte, 8));
  ...
  for (let i = 0; i < bytes.length; i += 8) {
    ...
    for (const byte of block) blockBits.push(...toBits(byte, 8));
    const decBlock = desDecryptBlock(blockBits, subkeys);
    for (let j = 0; j < 8; j++) {
      decrypted.push(fromBits(decBlock.slice(j * 8, j * 8 + 8)));
    }
  }
  ...
  const url = unpadded.map(b => String.fromCharCode(b)).join("");
