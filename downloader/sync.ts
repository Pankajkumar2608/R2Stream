...
async function loadManifest(r2: S3Client): Promise<Manifest> {
  try {
    const resp: GetObjectCommandOutput = await r2.send(
      new GetObjectCommand({ Bucket: config.r2.bucket, Key: config.keys.manifest })
    );
    const body = await resp.Body!.transformToString("utf-8");
    const data = JSON.parse(body) as Manifest;
    log.info(`Manifest loaded — ${Object.keys(data.tracks).length} tracks in library`);
    return data;
  } catch (err: any) {
...
async function downloadAndUpload(
  ...
  const titleWithoutArtist = rawTitle.toLowerCase().startsWith(artist.toLowerCase() + " - ")
  ...
  const trackUrl     = entry.webpage_url ?? entry.url ?? `https://www.youtube.com/watch?v=${videoId}`;
  ...
  try {
    ...
    const dlResult = await ytdlp([
      ...
      "--extract-audio",
      "--audio-format",      config.download.audioFormat,
      "--audio-quality",     config.download.audioQuality,
      "--embed-thumbnail",                    // embed cover art into MP3
      "--embed-metadata",                     // embed ID3 tags
      "--write-thumbnail",                    // also write cover as separate file
      "--concurrent-fragments", "5",          // faster fragment downloads
      "--no-warnings",
      "--quiet",
      ...cookieArgs(),
    ...
    if (dlResult.exitCode !== 0) {
      log.error(`Download failed [${rawTitle}]: ${dlResult.stderr?.slice(0, 200)}`);
      return null;
    }
 
    // ── Find downloaded files ─
    const files        = await readdir(tmpDir);
    const audioFile    = files.find(f => f.endsWith(`.${config.download.audioFormat}`));
    ...
    if (coverPath) {
      try {
        ...
        const ctype = ext === ".webp" ? "image/webp"
                    : ext === ".png"  ? "image/png"
                    :                   "image/jpeg";
...
async function sync(playlistUrls: string[]): Promise<void> {
  ...
  await pool(newEntries, async (entry) => {
    try {
